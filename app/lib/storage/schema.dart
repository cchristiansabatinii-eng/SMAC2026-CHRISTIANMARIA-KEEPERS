...
''',
  ...
  static const List<String> versionThreeStatements = [
    "ALTER TABLE members ADD COLUMN avatar_config_json TEXT NOT NULL DEFAULT '{}'",
  ];

  static const List<String> versionFourStatements = [
    r'''
...
WHERE member_key_ref IS NOT NULL AND role = 'adult'
ORDER BY created_at ASC, id ASC LIMIT 1
''',
  ];

  static const List<String> versionFiveStatements = [
    r'''
CREATE TABLE pending_family_invite_completion (
  singleton INTEGER PRIMARY KEY CHECK (singleton = 1),
...
''',
  ...
  static const List<String> versionSixStatements = [
    r'''
...
''',
  ...
  static List<String> statementsForUpgrade(int oldVersion, int newVersion) {
    ...
    if (oldVersion < 3 && newVersion >= 3) {
      statements.addAll(versionThreeStatements);
    }
    if (oldVersion < 4 && newVersion >= 4) {
      statements.addAll(versionFourStatements);
    }
    if (oldVersion < 5 && newVersion >= 5) {
      statements.addAll(versionFiveStatements);
    }
    if (oldVersion < 6 && newVersion >= 6) {
      statements.addAll(versionSixStatements);
    }
