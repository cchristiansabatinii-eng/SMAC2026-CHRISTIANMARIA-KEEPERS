...
final class PosixSecureBlobFileSystem
  ...
  final PosixPublicationStrategy? _publicationStrategyOverride;
  ...
  PosixPublicationStrategy get _publicationStrategy =>
      _publicationStrategyOverride ??
      publicationStrategyForAbi(currentPosixNativeAbi());
  ...
  Future<SecurePublishedBlob> finalize(
    ...
    try {
      ...
      if (_publicationStrategy == PosixPublicationStrategy.atomicMove) {
        _renameNoReplace(
          staged.attemptFd,
          _stagingLeaf,
        ...
        staged.sourcePublishedByMove = true;
      } else {
        _linkAt(
          staged.attemptFd,
          _stagingLeaf,
  ...
  _CleanupResult _cleanupAttempt(_PosixStagedBlob staged) {
    ...
    final cleanup = const FailureTotalCleanupRunner().run([
      ...
      PosixCleanupAction(PosixCleanupOperation.unlinkat, () {
        ...
        if (staged.sourcePublishedByMove) {
          final retainedLinks = _linkCount(staged.sourceFd);
          if (retainedLinks > 1) {
            throw FileSystemException(
              'Moved staging payload retained an unexpected link',
              _stagingLeaf,
            );
          }
          return;
...
final class _PosixStagedBlob implements SecureStagedBlob {
  ...
  bool consumed = false;
  bool sourcePublishedByMove = false;
}
