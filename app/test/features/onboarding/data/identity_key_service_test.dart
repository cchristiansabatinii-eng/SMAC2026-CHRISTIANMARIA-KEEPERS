...
void main() {
  ...
  test('imports an exact family key once under its stable reference', () async {
    final store = _MemorySecureValueStore();
    final service = IdentityKeyService(store);
    final familyKey = List<int>.filled(32, 19);

    final imported = await service.importFamilyKey(
      familyId: 'family-1',
      familyKey: familyKey,
    );
    familyKey[0] = 99;
    final repeated = await service.importFamilyKey(
      familyId: 'family-1',
      familyKey: List<int>.filled(32, 19),
    );

    expect(imported.reference, 'keepers.family.family-1.entry-key.v1');
    expect(imported.wasCreated, isTrue);
    expect(repeated.reference, imported.reference);
    expect(repeated.wasCreated, isFalse);
    expect(await service.resolve(imported.reference), List<int>.filled(32, 19));
  });

  test(
    'creates a member key on-device once under its stable reference',
    () async {
      final store = _MemorySecureValueStore();
      final service = IdentityKeyService(
        store,
      ...
      final created = await service.createMemberKey(memberId: 'member-1');
      final repeated = await service.createMemberKey(memberId: 'member-1');

      expect(created.reference, 'keepers.member.member-1.entry-key.v1');
      expect(created.wasCreated, isTrue);
      expect(repeated.reference, created.reference);
      expect(repeated.wasCreated, isFalse);
      expect(
        await service.resolve(created.reference),
        List<int>.filled(32, 23),
      );
    },
  );

  test(
    'family key import rejects wrong length and conflicting local key',
    () async {
      final store = _MemorySecureValueStore();
      final service = IdentityKeyService(store);
      final imported = await service.importFamilyKey(
        familyId: 'family-1',
        familyKey: List<int>.filled(32, 29),
      );

      await expectLater(
        service.importFamilyKey(
          familyId: 'family-1',
          familyKey: List<int>.filled(31, 29),
        ),
        throwsArgumentError,
      );
      await expectLater(
        service.importFamilyKey(
          familyId: 'family-1',
          familyKey: List<int>.filled(32, 31),
        ),
        throwsStateError,
      );

      expect(
        await service.resolve(imported.reference),
        List<int>.filled(32, 29),
      );
    },
  );
