...
void main() {
  testWidgets('shows every format and defaults to Weekly Reveal', (
    tester,
  ) async {
    ...
    expect(
      find.text('Only you can open this memory on this device.'),
      findsOneWidget,
    );
    expect(
      find.text('Sealed for your family’s future reveal.'),
      findsOneWidget,
    );
    expect(
      find.text('Sealed with the family key for a future milestone.'),
      findsOneWidget,
    );
  ...
  testWidgets('text plus optional caption enables Seal memory', (tester) async {
    final harness = CaptureTestHarness();
    addTearDown(harness.dispose);
    await tester.pumpWidget(harness.sheet());
    await tester.pump();
    ...
    final save = tester.widget<FilledButton>(
      find.widgetWithText(FilledButton, 'Seal memory'),
    );
  ...
  testWidgets('saving shows progress and returns the persisted entry once', (
    ...
    await tester.pump();
    await tester.ensureVisible(find.text('Seal memory'));
    await tester.tap(find.widgetWithText(FilledButton, 'Seal memory'));
    await tester.pump();
  ...
  testWidgets('committed cleanup stays open and retries before returning', (
    ...
    expect(find.text('Completions: 0'), findsOneWidget);
    expect(
      find.text('The memory is sealed, but draft cleanup needs attention.'),
      findsOneWidget,
    );
    expect(
  ...
  testWidgets('failed save keeps the draft and Try again succeeds', (
    ...
    expect(
      find.text('This memory could not be sealed. Your draft is still here.'),
      findsOneWidget,
    );
    ...
    expect(
      tester.widget<TextField>(find.byKey(const Key('memory-text'))).enabled,
      isTrue,
    );
  ...
  testWidgets('null dismissal resets privacy before the next opening', (
    ...
    await tester.tap(find.text('Add a memory'));
    await tester.pumpAndSettle();
    await tester.ensureVisible(find.text('Private Journal'));
